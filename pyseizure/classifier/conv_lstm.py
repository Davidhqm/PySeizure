...
class ConvLSTM(Classifier):
    ...
    def _init_loaders_and_model(self, reinit_keys: bool = True):
        ...
        self.model = ConvLSTMModel(self.dev, self.config).to(self.dev)
    ...
    def _train(self, trial):
        config = {
            ...
            'lstm_hd': trial.suggest_categorical(
                'lstm_hd', [2**i for i in range(6, 8)]),
            'lstm_num_layers': trial.suggest_categorical(
                'lstm_num_layers', [2, 3, 4, 5]),
            # 'f1_hd': trial.suggest_categorical(
            #     'f1_hd', [2**i for i in range(5, 9)]),
            'f2_hd': trial.suggest_categorical(
                'f2_hd', [2**i for i in range(6, 8)]),
            'f3_hd': trial.suggest_categorical(
                'f3_hd', [2**i for i in range(5, 7)]),
        ...
        model = ConvLSTMModel(self.dev, config).to(self.dev)
        test_dataset = ConvDataset(self.valid_samples,
                                   self.label_encoder,
                                   self.feature_selector,
                                   self.normaliser,
                                   ...
                                   binary=self.binary,
                                   artefacts=self.artefacts)
        train_dataset = ConvDataset(self.train_samples,
                                    self.label_encoder,
                                    self.feature_selector,
                                    self.normaliser,
                                    batch_size=config['batch_size'],
    ...
    def tune(self, config: Dict):
        ...
        study = optuna.create_study(
            ...
            study_name=f'{classifier_name.CONV_LSTM.value.lower()}'
        ...
        with open(f'output/models/{classifier_name.CONV_LSTM.value.lower()}'
    ...
    def train(self):
        writer = SummaryWriter(
            f"output/runs/{classifier_name.CONV_LSTM.value.lower()}"
            f"_{self.dataset_name}_"
            ...
            for epoch in range(self.epoch):
                ...
                if writer is not None:
                    ...
                    fig.savefig(f"output/train/"
                                f"{classifier_name.CONV_LSTM.value.lower()}"
                                f"_{epoch}.png")
                    plt.close()
                    pbar.set_postfix(
        ...
        self.model = torch.load(
            f'output/models/{classifier_name.CONV_LSTM.value.lower()}'
    ...
    def evaluate(self,
        ...
        if writer is not None:
            ...
            with open(f"output/eval/conv_lstm_"
...
class ConvLSTMModel(nn.Module):
    def __init__(self, dev: device, config: dict = {
        ...
        self.lstm = nn.LSTM(input_size=self.conv_out_size,
                            hidden_size=self.config['lstm_hd'],
                            num_layers=self.config['lstm_num_layers'],
                            batch_first=True)

        linear1 = nn.Linear(in_features=self.config['lstm_hd'],
                            out_features=self.config['f2_hd'])
        nn.init.kaiming_normal_(linear1.weight, nonlinearity='leaky_relu')

        linear2 = nn.Linear(in_features=self.config['f2_hd'],
                            out_features=self.config['f3_hd'])
        ...
        self.fc2 = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Dropout(p=0.5),
            nn.BatchNorm1d(self.config['f2_hd']),

            linear2,
            nn.LeakyReLU(),
            nn.Dropout(p=0.5),
            nn.BatchNorm1d(self.config['f3_hd'])
        )

        self.output = nn.Linear(in_features=self.config['f3_hd'],
                                out_features=2)
        nn.init.kaiming_normal_(self.output.weight, nonlinearity='leaky_relu')

    def init_hidden(self, x):
        h0 = torch.zeros(self.config['lstm_num_layers'], x.size(0),
                         self.config['lstm_hd']).to(self.dev)
        c0 = torch.zeros(self.config['lstm_num_layers'], x.size(0),
                         self.config['lstm_hd']).to(self.dev)
    ...
    def forward(self, x):
        x = self.conv(x)
        # x = x.view(-1, (self.config['conv3_hd'] *
        #                 self.conv_out_size))
        # x = self.fc1(x)
        x, _ = self.lstm(x, self.init_hidden(x))
        x = self.fc2(x[:, -1, :])
